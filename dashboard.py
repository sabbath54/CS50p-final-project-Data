...
fig = px.bar(top_skills_count, x=top_skills_count.values, y=top_skills_count.index, orientation='h', title="Top 15 Required Skills")
fig.update_layout(xaxis_title="Appearance count", yaxis_title="Skill name")
...
top_16_to_50_skills = skills_count.nlargest(50)[15:][::-1].reset_index().rename(columns={"index": "Skill name", 0: "Appearance count"})
top_16_to_50_skills.index = range(16, len(top_16_to_50_skills) + 16)
st.write("**Top 16-50 skills and their appearance count:**")
st.dataframe(top_16_to_50_skills, height=500)

# Display jobs
st.write("**Jobs:**")
df_selection.rename(columns={"listing_name": "Listing name", "categories": "Categorie", "seniority": "Seniority", "required_skills": "Required skills"}, inplace=True)
